...
source("src/utils/phenoscanner.R")
...
source("src/utils/annotation_utils.R")
...
doc_folder <- file.path("docs/regenie", md_base)
...
write(
  x = paste0("| SNP | chr | bp | ref | ref freq | beta | se | p | n | Ensembl | Phenoscanner | freq geno | b joint | b joint se | p joint | ld r |"), 
  file = md_file, 
  append = T
)
write(
...
if (nrow(cojo_not_hla) > 0) {
  ...
  for (i in 1:nrow(cojo_not_hla)) {
    ...
    write(
      x = glue("| {snp} | {chr} | {bp} | {ref} | {ref_freq} | {beta} | {se} | {p} | {n} | {geneLabel} | {phenoScannerLabel} | {freq_geno} | {b_j} | {b_j_se} | {p_j} | {ld_r} |"), 
      file = md_file, 
      append = T
    )
