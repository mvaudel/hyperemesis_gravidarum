...
binary_phenotypes <- c("perinatal_death", "spontaneous_delivery", "hospitalized_pre_eclampsia", "early_preeclampsia", "preeclampsia", "preeclampsia_serious", "diabetes_before", "diabetes_during", "diabetes_during_not_before", "diabetes")
...
numeric_phenotypes <- c("n_previous_stillbirths", "n_previous_live_births", "previous_miscarriages", "previous_miscarriages_before_12w", "previous_miscarriages_12_23w", "pregnancy_duration", "placental_weight", "z_placental_weight", "z_placental_weight_pregnancy_duration", "weight_birth", "z_weight_birth", "z_weight_birth_pregnancy_duration", "hc_birth", "z_hc_birth", "z_hc_birth_length_birth", "z_hc_birth_weight_birth", "z_placental_weight_weight_birth", "preeclampsia_quant", "mother_age_at_menarche", "mother_age_15w", "mother_height", "mother_weight_beginning_self", "mother_weight_15w", "mother_weight_end", "mother_bmi_beginning", "mother_bmi_15w", "mother_bmi_end", "mother_bmi_beginning_log", "mother_bmi_15w_log", "mother_bmi_end_log", "mother_weight_gain_15w", "mother_weight_gain_15w_relative", "mother_weight_gain_end", "mother_weight_gain_end_relative")
...
phenotypes <- c("perinatal_death", "spontaneous_delivery", "hospitalized_pre_eclampsia", "early_preeclampsia", "preeclampsia", "preeclampsia_serious", "diabetes_before", "diabetes_during", "diabetes_during_not_before", "diabetes", "n_previous_stillbirths", "n_previous_live_births", "previous_miscarriages", "previous_miscarriages_before_12w", "previous_miscarriages_12_23w", "pregnancy_duration", "placental_weight", "z_placental_weight", "z_placental_weight_pregnancy_duration", "weight_birth", "z_weight_birth", "z_weight_birth_pregnancy_duration", "hc_birth", "z_hc_birth", "z_hc_birth_length_birth", "z_hc_birth_weight_birth", "z_placental_weight_weight_birth", "preeclampsia_quant", "mother_age_at_menarche", "mother_age_15w", "mother_height", "mother_weight_beginning_self", "mother_weight_15w", "mother_weight_end", "mother_bmi_beginning", "mother_bmi_15w", "mother_bmi_end", "mother_bmi_beginning_log", "mother_bmi_15w_log", "mother_bmi_end_log", "mother_weight_gain_15w", "mother_weight_gain_15w_relative", "mother_weight_gain_end", "mother_weight_gain_end_relative")
...
lm_coefs_mother <- lm_coefs_mother %>% 
   separate(
     plink_id, 
     into = c("id", "allele"), 
     extra = "drop", 
...
meta_results <- read.table(
  file = "utils/top_hits_proxies",
  header = T
) %>% 
  mutate(
    id = ifelse(is.na(proxy), snp, proxy),
    allele_meta = ifelse(is.na(proxy), a1, proxy_a1),
  ) %>% 
  filter(
    !id %in% c("rs184084750", "rs34935290")
  )
...
plot_data <- lm_coefs_mother %>% 
  ...
  left_join(
    meta_results %>% 
      select(
        id, allele_meta, beta_meta = beta, se_meta = se, p_meta = p
      ),
    by = "id"
  ) %>% 
  mutate(
...
plot <- ggplot(
  ...
  geom_segment(
    mapping = aes(
      x = beta_meta - qnorm(0.975) * se_meta,
      xend = beta_meta + qnorm(0.975) * se_meta,
      y = beta_swapped,
      yend = beta_swapped
    ),
    color = "grey40"
  ...
  geom_segment(
    mapping = aes(
      x = beta_meta,
      xend = beta_meta,
      y = beta_swapped - qnorm(0.975) * se_meta,
      yend = beta_swapped + qnorm(0.975) * se_meta
    ),
    color = "grey40"
  ) +
  geom_point(
...
for (phenotype_i in 1:length(phenotypes)) {
  ...
  png(
    filename = glue("plots/{phenotype}_{hg_label}_{term_level}_prs.png"),
    width = 900,
    height = 600
  )
  grid.draw(plot)
...
exposure_data <- data.frame(
  SNP = c("rs1058587"),
  beta.exposure = c(0.2783),
  se.exposure = c(0.0181),
  effect_allele.exposure = c("C"),
  other_allele.exposure = c("G"),
  eaf.exposure = c(0.748)
)
