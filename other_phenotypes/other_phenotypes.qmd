...
Process parental anthropometrics
...
phenotypes <- c("placental_weight", "weight_birth", "hc_birth")
...
binary_phenotypes <- c("perinatal_death", "hospitalized_pre_eclampsia", "early_preeclampsia", "preeclampsia", "preeclampsia_serious", "diabetes", "diabetes_before", "diabetes_during", "diabetes_during_not_before")
...
numeric_phenotypes <- c("pregnancy_duration", "placental_weight",  "z_placental_weight_pregnancy_duration", "weight_birth", "z_weight_birth_pregnancy_duration", "hc_birth", "z_hc_birth_pregnancy_duration", "z_hc_birth_length_birth", "mother_age_at_menarche", "mother_age_15w", "mother_weight_beginning_self", "mother_weight_15w", "mother_weight_end", "mother_bmi_beginning", "mother_bmi_15w", "mother_bmi_end")
...
lm_coefs_mother_merged <- lm_coefs_mother %>% 
  ...
  mutate(
    ...
    hg_increasing_allele_frequency = ifelse(allele == hg_increasing_allele, allele_frequency, 1-allele_frequency)
  )
...
i <- !is.na(mfr_data$child_sentrix_id) & !is.na(mfr_data$early_preeclampsia) & mfr_data$early_preeclampsia == "Yes"

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & !is.na(mfr_data$early_preeclampsia) & mfr_data$early_preeclampsia == "Yes"

length(unique(mfr_data$mother_sentrix_id[i]))

i <- !is.na(mfr_data$child_sentrix_id) & !is.na(mfr_data$preeclampsia) & mfr_data$preeclampsia == "Light"

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & !is.na(mfr_data$preeclampsia) & mfr_data$preeclampsia == "Light"

length(unique(mfr_data$mother_sentrix_id[i]))

i <- !is.na(mfr_data$child_sentrix_id) & !is.na(mfr_data$preeclampsia) & mfr_data$preeclampsia == "Serious"

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & !is.na(mfr_data$preeclampsia) & mfr_data$preeclampsia == "Serious"

length(unique(mfr_data$mother_sentrix_id[i]))

i <- !is.na(mfr_data$child_sentrix_id) & !is.na(mfr_data$preeclampsia) & mfr_data$preeclampsia == "Unspecified"

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & !is.na(mfr_data$preeclampsia) & mfr_data$preeclampsia == "Unspecified"

length(unique(mfr_data$mother_sentrix_id[i]))

i <- !is.na(mfr_data$child_sentrix_id) & !is.na(mfr_data$eclampsia_total) & mfr_data$eclampsia_total == "Yes"

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & !is.na(mfr_data$eclampsia_total) & mfr_data$eclampsia_total == "Yes"
...
mfr_data <- mfr_data %>% 
  mutate(
    case = ifelse(
      !is.na(early_preeclampsia) | !is.na(preeclampsia) | !is.na(eclampsia_total), 1, 0
    )
  )

i <- !is.na(mfr_data$child_sentrix_id) & mfr_data$case == 1

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & mfr_data$case == 1

length(unique(mfr_data$mother_sentrix_id[i]))

i <- !is.na(mfr_data$child_sentrix_id) & mfr_data$case == 0 & mfr_data$pregnancy_duration < 37 * 7

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & mfr_data$case == 0 & mfr_data$pregnancy_duration < 37 * 7

length(unique(mfr_data$mother_sentrix_id[i]))

i <- !is.na(mfr_data$child_sentrix_id) & mfr_data$case == 1 & mfr_data$pregnancy_duration < 37 * 7

length(unique(mfr_data$child_sentrix_id[i]))
length(unique(mfr_data$preg_id[i]))

i <- !is.na(mfr_data$mother_sentrix_id) & mfr_data$case == 1 & mfr_data$pregnancy_duration < 37 * 7
