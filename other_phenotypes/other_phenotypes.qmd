...
meta_results <- read.table(
  file = "utils/look_up_snps_24.02.21",
  sep = "\t",
  header = T
)  %>% 
  clean_names() %>% 
...
plot_data <- lm_coefs_mother %>% 
  ...
  left_join(
    meta_results %>% 
      select(
        id, allele_meta = allele1, beta_meta = beta, se_meta = se, p_meta = p
      ),
    by = "id"
  ) %>% 
  mutate(
